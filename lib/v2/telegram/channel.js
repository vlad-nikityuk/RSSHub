...
const agentRegex = /ДАННОЕ СООБЩЕНИЕ (МАТЕРИАЛ) СОЗДАНО И (ИЛИ) РАСПРОСТРАНЕНО ИНОСТРАННЫМ СРЕДСТВОМ МАССОВОЙ ИНФОРМАЦИИ, ВЫПОЛНЯЮЩИМ ФУНКЦИИ ИНОСТРАННОГО АГЕНТА, И (ИЛИ) РОССИЙСКИМ ЮРИДИЧЕСКИМ ЛИЦОМ, ВЫПОЛНЯЮЩИМ ФУНКЦИИ ИНОСТРАННОГО АГЕНТА/i;
            ...
            list
                .map((index, item) => {
                    ...
                    if (messageTextObj.length > 0 && !titleCompleteFlag) {
                        ...
                        const trimmedTitleText = _messageTextObj.text().trim().replace(agentRegex, '');
                        const lbr = trimmedTitleText.indexOf("\n");

                        var resultTitleText = "";
                        if (lbr > 10 && lbr <= config.titleLengthLimit) {
                            resultTitleText = trimmedTitleText.substr(0, lbr);
                        } else {
                            resultTitleText = trimmedTitleText.replace(/\n/g, ' ').trim();
                        }

                        messageTitle += (messageTitle && resultTitleText ? ': ' : '') + resultTitleText;
                    }
                    ...
                    description = description.replace(agentRegex, '');
