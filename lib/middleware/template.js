        ...
        if (ctx.state.data) {
            ...
            ctx.state.data.item &&
                ctx.state.data.item.forEach((item) => {
                    if (item.title) {
                        var agentRegex = /ДАННОЕ СООБЩЕНИЕ (МАТЕРИАЛ) СОЗДАНО И (ИЛИ) РАСПРОСТРАНЕНО ИНОСТРАННЫМ СРЕДСТВОМ МАССОВОЙ ИНФОРМАЦИИ, ВЫПОЛНЯЮЩИМ ФУНКЦИИ ИНОСТРАННОГО АГЕНТА, И (ИЛИ) РОССИЙСКИМ ЮРИДИЧЕСКИМ ЛИЦОМ, ВЫПОЛНЯЮЩИМ ФУНКЦИИ ИНОСТРАННОГО АГЕНТА/i;
                        item.title = item.title.replace(agentRegex, '');
                        ...
                        for (let length = 0, i = 0; i < item.title.length; i++) {
                            ...
                            if (length > config.titleLengthLimit) {
                                let cutd = item.title.slice(0, i);
                                let lastSpace = cutd.lastIndexOf(" ");
                                let cutIndex =  lastSpace == -1 ? i : lastSpace;
                                item.title = `${item.title.slice(0, cutIndex)}...`;
                                break;
                            }
                        }
                    }
